...
* **APIs That Call `SetLastError()`**: These APIs now return a
  ...
  and the associated last-error code, ensuring the **error state is preserved**.
...
As of **v6.3.0**, `package:win32` no longer ships a native DLL. Instead,
bindings that use `GetLastError()` now pre-resolve it before each API call,
making error capture reliable in practice without requiring any native code.
...
[Leak Tracking]: ../advanced/leak-tracking.mdx
[Functions]: ../core/functions.mdx
[COM Methods]: ../advanced/com/methods.mdx
