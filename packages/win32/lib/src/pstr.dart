...
/// A wrapper for the [PSTR], used in Windows APIs.
///
/// This class simplifies working with [PSTR]s by providing automatic memory
...
final class Pstr implements Finalizable {
  /// Creates a new [Pstr] with the provided [string].
  ///
  ...
  factory Pstr(String string) => Pstr._fromPointer(string.toPSTR());

  /// Creates a [Pstr] that can hold [length] characters, including the NUL
  /// terminator.
  ///
  ...
  /// memory when no longer used.
  @pragma('vm:prefer-inline')
  factory Pstr.allocate([int length = 1]) =>
      Pstr._fromPointer(loggingCalloc<BYTE>(length).cast());

  Pstr._fromPointer(this.ptr) {
    _finalizer.attach(
      this,
      ptr.cast(),
      detach: this,
  ...
  /// Creates a [Pstr] from an existing [PSTR].
  ///
  ...
  factory Pstr.fromPointer(PSTR ptr) {
    if (ptr.isNull) {
      throw ArgumentError.value(ptr, 'ptr', 'Pointer cannot be null');
    }

    return Pstr._fromPointer(ptr);
  }
  ...
  /// The length of the [PSTR] in bytes, not including the terminating NUL
  /// character.
  @pragma('vm:prefer-inline')
  int get byteLength => length;

  /// Creates a new [Pstr] that is a copy of the current [Pstr].
  Pstr clone() {
    // Allocate memory for a PSTR of sufficient length (including the NUL
    // terminator) to hold the string.
    final pstr = loggingCalloc<BYTE>(length + 1);
    pstr
        .asTypedList(length + 1)
        .setAll(0, ptr.cast<BYTE>().asTypedList(length + 1));
    return Pstr._fromPointer(pstr.cast());
  }

  /// Detaches this instance from the [NativeFinalizer].
  ///
  /// Use this method to manually manage the memory of the [PSTR].
  ...
  /// the resources associated with the [PSTR].
  ...
  /// Whether this [PSTR] is empty.
  @pragma('vm:prefer-inline')
  bool get isEmpty => length == 0;

  /// The number of UTF-8 code units in this [PSTR], not including the
  ...
  /// Concatenates this [Pstr] with [other] and returns the result.
  ///
  /// Throws a [WindowsException] if the concatenation fails.
  Pstr operator +(Pstr other) {
    final pstr = loggingMalloc<BYTE>(length + other.length + 1);
    pstr.asTypedList(length + other.length)
      ..setAll(0, ptr.cast<BYTE>().asTypedList(length))
      ..setAll(length, other.ptr.cast<BYTE>().asTypedList(other.length));
    pstr[length + other.length] = 0;
    return Pstr._fromPointer(pstr.cast());
  }
