...
/// A wrapper for the [PWSTR], used in Windows APIs.
///
/// This class simplifies working with [PWSTR]s by providing automatic memory
...
final class Pwstr implements Finalizable {
  /// Creates a new [Pwstr] with the provided [string].
  ///
  ...
  factory Pwstr(String string) => Pwstr._fromPointer(string.toPWSTR());

  /// Creates a [Pwstr] that can hold [length] characters, including the NUL
  /// terminator.
  ///
  ...
  /// memory when no longer used.
  @pragma('vm:prefer-inline')
  factory Pwstr.allocate([int length = 1]) =>
      Pwstr._fromPointer(loggingCalloc<WCHAR>(length).cast());

  Pwstr._fromPointer(this.ptr) {
    _finalizer.attach(
      this,
      ptr.cast(),
      detach: this,
  ...
  /// Creates a [Pwstr] from an existing [PWSTR].
  ///
  ...
  /// the resources associated with the [PWSTR].
  ...
  /// Whether this [PWSTR] is empty.
  @pragma('vm:prefer-inline')
  bool get isEmpty => length == 0;

  /// The number of UTF-16 code units in this [PWSTR], not including the
  ...
  /// Concatenates this [Pwstr] with [other] and returns the result.
  ///
  /// Throws a [WindowsException] if the concatenation fails.
  Pwstr operator +(Pwstr other) {
    final pwstr = loggingMalloc<WCHAR>(length + other.length + 1);
    pwstr.asTypedList(length + other.length)
      ..setAll(0, ptr.cast<WCHAR>().asTypedList(length))
      ..setAll(length, other.ptr.cast<WCHAR>().asTypedList(other.length));
    pwstr[length + other.length] = 0;
    return Pwstr._fromPointer(pwstr.cast());
  }
  ...
  List<String> toDartStringList(int maxLength) =>
      ptr.toDartStringList(maxLength);

  @override
  String toString() => 'Pwstr(ptr: $ptr)';
}
