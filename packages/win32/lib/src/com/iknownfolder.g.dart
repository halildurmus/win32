...
class IKnownFolder extends IUnknown implements ComInterface {
  ...
  factory IKnownFolder.from(IUnknown interface) => interface.queryInterface();
  ...
  late final _GetPathFn =
      ...
      >();
  late final _SetPathFn = _vtable
      .SetPath.asFunction<int Function(VTablePointer, int, Pointer<Utf16>)>();
  ...
  Pointer<GUID> getId() {
    final pkfid = adaptiveCalloc<GUID>();
  ...
  T getShellItem<T extends IUnknown>(int dwFlags) {
    ...
    final hr$ = HRESULT(_GetShellItemFn(ptr, dwFlags, riid, ppv));
    free(riid);
    if (hr$.isError) {
      free(ppv);
      throw WindowsException(hr$);
  ...
  Pointer<GUID> getFolderType() {
    final pftid = adaptiveCalloc<GUID>();
  ...
  int getRedirectionCapabilities() {
    final pCapabilities = adaptiveCalloc<Uint32>();
  ...
  Pointer<KNOWNFOLDER_DEFINITION> getFolderDefinition() {
    final pKFD = adaptiveCalloc<KNOWNFOLDER_DEFINITION>();
