...
final IID_IPropertyDescriptionList = GUID.fromComponents(
  ...
  0xc39b,
  0x4b26,
  .fromList(const [0x81, 0x7f, 0x1, 0x19, 0x67, 0xd3, 0x44, 0xe]),
);
...
class IPropertyDescriptionList extends IUnknown implements ComInterface {
  ...
  factory IPropertyDescriptionList.from(IUnknown interface) =>
  ...
  int getCount() {
    final pcElem = adaptiveCalloc<Uint32>();
  ...
  T getAt<T extends IUnknown>(int iElem) {
    ...
    final hr$ = HRESULT(_GetAtFn(ptr, iElem, riid, ppv));
    free(riid);
    if (hr$.isError) {
      free(ppv);
      throw WindowsException(hr$);
