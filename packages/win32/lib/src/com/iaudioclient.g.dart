...
final IID_IAudioClient = GUID.fromComponents(
  ...
  0xdbfa,
  0x4c32,
  .fromList(const [0xb1, 0x78, 0xc2, 0xf5, 0x68, 0xa7, 0x3, 0xb2]),
);
...
class IAudioClient extends IUnknown implements ComInterface {
  ...
  factory IAudioClient.from(IUnknown interface) => interface.queryInterface();
  ...
  late final _SetEventHandleFn =
      _vtable.SetEventHandle.asFunction<int Function(VTablePointer, Pointer)>();
  ...
  int getBufferSize() {
    final pNumBufferFrames = adaptiveCalloc<Uint32>();
  ...
  int getStreamLatency() {
    final phnsLatency = adaptiveCalloc<Int64>();
  ...
  int getCurrentPadding() {
    final pNumPaddingFrames = adaptiveCalloc<Uint32>();
  ...
  Pointer<WAVEFORMATEX> getMixFormat() {
    final ppDeviceFormat = adaptiveCalloc<Pointer<WAVEFORMATEX>>();
  ...
  T getService<T extends IUnknown>() {
    ...
    final hr$ = HRESULT(_GetServiceFn(ptr, riid, ppv));
    free(riid);
    if (hr$.isError) {
      free(ppv);
      throw WindowsException(hr$);
...
base class IAudioClientVtbl extends Struct {
  ...
  external Pointer<
    NativeFunction<Int32 Function(VTablePointer this$, Pointer eventHandle)>
  >
  SetEventHandle;
  external Pointer<
    NativeFunction<
