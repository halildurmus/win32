...
class IMoniker extends IPersistStream implements ComInterface {
  ...
  factory IMoniker.from(IUnknown interface) => interface.queryInterface();
  ...
  IMoniker? composeWith(IMoniker? pmkRight, bool fOnlyIfNotGeneric) {
    final ppmkComposite = adaptiveCalloc<VTablePointer>();
    final hr$ = HRESULT(
      _ComposeWithFn(
        ptr,
  ...
  IEnumMoniker? enum$(bool fForward) {
    final ppenumMoniker = adaptiveCalloc<VTablePointer>();
  ...
  IMoniker? inverse() {
    final ppmk = adaptiveCalloc<VTablePointer>();
  ...
  IMoniker? commonPrefixWith(IMoniker? pmkOther) {
    final ppmkPrefix = adaptiveCalloc<VTablePointer>();
    final hr$ = HRESULT(
  ...
  IMoniker? relativePathTo(IMoniker? pmkOther) {
    final ppmkRelPath = adaptiveCalloc<VTablePointer>();
    final hr$ = HRESULT(
  ...
  PWSTR getDisplayName(IBindCtx? pbc, IMoniker? pmkToLeft) {
    final ppszDisplayName = adaptiveCalloc<Pointer<Utf16>>();
    final hr$ = HRESULT(
      _GetDisplayNameFn(
        ptr,
        pbc?.ptr ?? nullptr,
  ...
  int isSystemMoniker() {
    final pdwMksys = adaptiveCalloc<Uint32>();
