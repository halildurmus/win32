...
final IID_IPropertyStore = GUID.fromComponents(
  ...
  0x8cf2,
  0x4446,
  .fromList(const [0x8d, 0x2, 0xcd, 0xba, 0x1d, 0xbd, 0xcf, 0x99]),
);
...
class IPropertyStore extends IUnknown implements ComInterface {
  ...
  factory IPropertyStore.from(IUnknown interface) => interface.queryInterface();
  ...
  int getCount() {
    final cProps = adaptiveCalloc<Uint32>();
  ...
  Pointer<PROPERTYKEY> getAt(int iProp) {
    final pkey = adaptiveCalloc<PROPERTYKEY>();
  ...
  Pointer<PROPVARIANT> getValue(Pointer<PROPERTYKEY> key) {
    final pv = adaptiveCalloc<PROPVARIANT>();
