...
class ITypeInfo extends IUnknown implements ComInterface {
  ...
  factory ITypeInfo.from(IUnknown interface) => interface.queryInterface();
  ...
  Pointer<TYPEATTR> getTypeAttr() {
    final ppTypeAttr = adaptiveCalloc<Pointer<TYPEATTR>>();
  ...
  Pointer<FUNCDESC> getFuncDesc(int index) {
    final ppFuncDesc = adaptiveCalloc<Pointer<FUNCDESC>>();
  ...
  Pointer<VARDESC> getVarDesc(int index) {
    final ppVarDesc = adaptiveCalloc<Pointer<VARDESC>>();
  ...
  int getRefTypeOfImplType(int index) {
    final pRefType = adaptiveCalloc<Uint32>();
  ...
  IMPLTYPEFLAGS getImplTypeFlags(int index) {
    final pImplTypeFlags = adaptiveCalloc<Int32>();
  ...
  T createInstance<T extends IUnknown>(IUnknown? pUnkOuter) {
    ...
    final ppvObj = adaptiveCalloc<Pointer>();
    final hr$ = HRESULT(
      _CreateInstanceFn(ptr, pUnkOuter?.ptr ?? nullptr, riid, ppvObj),
    );
    free(riid);
    if (hr$.isError) {
      free(ppvObj);
  ...
  BSTR getMops(int memid) {
    final pBstrMops = adaptiveCalloc<Pointer<Utf16>>();
