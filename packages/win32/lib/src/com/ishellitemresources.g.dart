...
class IShellItemResources extends IUnknown implements ComInterface {
  ...
  int getAttributes() {
    final pdwAttributes = adaptiveCalloc<Uint32>();
  ...
  int getSize() {
    final pullSize = adaptiveCalloc<Uint64>();
  ...
  PWSTR getResourceDescription(Pointer<SHELL_ITEM_RESOURCE> pcsir) {
    final ppszDescription = adaptiveCalloc<Pointer<Utf16>>();
  ...
  T openResource<T extends IUnknown>(Pointer<SHELL_ITEM_RESOURCE> pcsir) {
    ...
    final hr$ = HRESULT(_OpenResourceFn(ptr, pcsir, riid, ppv));
    free(riid);
    if (hr$.isError) {
      free(ppv);
      throw WindowsException(hr$);
  ...
  T createResource<T extends IUnknown>(Pointer<SHELL_ITEM_RESOURCE> pcsir) {
    ...
    final riid = companion.iid.toNative();
    final ppv = adaptiveCalloc<Pointer>();
    final hr$ = HRESULT(_CreateResourceFn(ptr, pcsir, riid, ppv));
    free(riid);
    if (hr$.isError) {
      free(ppv);
      throw WindowsException(hr$);
