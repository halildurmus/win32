...
class IShellFolder extends IUnknown implements ComInterface {
  ...
  factory IShellFolder.from(IUnknown interface) => interface.queryInterface();
  ...
  T bindToObject<T extends IUnknown>(Pointer<ITEMIDLIST> pidl, IBindCtx? pbc) {
    ...
    final hr$ = HRESULT(
      _BindToObjectFn(ptr, pidl, pbc?.ptr ?? nullptr, riid, ppv),
    );
    free(riid);
    if (hr$.isError) {
      free(ppv);
  ...
  T bindToStorage<T extends IUnknown>(Pointer<ITEMIDLIST> pidl, IBindCtx? pbc) {
    ...
    final ppv = adaptiveCalloc<Pointer>();
    final hr$ = HRESULT(
      _BindToStorageFn(ptr, pidl, pbc?.ptr ?? nullptr, riid, ppv),
    );
    free(riid);
    if (hr$.isError) {
      free(ppv);
  ...
  T createViewObject<T extends IUnknown>(HWND hwndOwner) {
    final companion = ComInterface.type<T>();
    final riid = companion.iid.toNative();
    final ppv = adaptiveCalloc<Pointer>();
    final hr$ = HRESULT(_CreateViewObjectFn(ptr, hwndOwner, riid, ppv));
    free(riid);
    if (hr$.isError) {
      free(ppv);
      throw WindowsException(hr$);
  ...
  T getUIObjectOf<T extends IUnknown>(
    ...
    final ppv = adaptiveCalloc<Pointer>();
    final hr$ = HRESULT(
      _GetUIObjectOfFn(ptr, hwndOwner, cidl, apidl, riid, nullptr, ppv),
    );
    free(riid);
    if (hr$.isError) {
      free(ppv);
