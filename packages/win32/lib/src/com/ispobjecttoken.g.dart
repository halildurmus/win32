...
class ISpObjectToken extends ISpDataKey implements ComInterface {
  ...
  factory ISpObjectToken.from(IUnknown interface) => interface.queryInterface();
  ...
  late final _RemoveStorageFileNameFn =
      _vtable.RemoveStorageFileName.asFunction<
        int Function(VTablePointer, Pointer<GUID>, Pointer<Utf16>, int)
      >();
  ...
  /// Throws a [WindowsException] on failure.
  PWSTR getId() {
    final ppszCoMemTokenId = adaptiveCalloc<Pointer<Utf16>>();
  ...
  ISpObjectTokenCategory? getCategory() {
    final ppTokenCategory = adaptiveCalloc<VTablePointer>();
  ...
  PWSTR getStorageFileName(
    ...
    final ppszFilePath = adaptiveCalloc<Pointer<Utf16>>();
    final hr$ = HRESULT(
  ...
  /// Throws a [WindowsException] on failure.
  @pragma('vm:prefer-inline')
  void matchesAttributes(PCWSTR pszAttributes, Pointer<Int32> pfMatches) {
