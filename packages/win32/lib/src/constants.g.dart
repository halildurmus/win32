...
const COMPOSITIONOBJECT_READ = 1;

const COMPOSITIONOBJECT_WRITE = 2;

const COMPOSITION_STATS_MAX_TARGETS = 256;
...
const D3D11_16BIT_INDEX_STRIP_CUT_VALUE = 65535;

const D3D11_1_UAV_SLOT_COUNT = 64;

const D3D11_2_TILED_RESOURCE_TILE_SIZE_IN_BYTES = 65536;

const D3D11_32BIT_INDEX_STRIP_CUT_VALUE = 4294967295;

const D3D11_4_VIDEO_DECODER_HISTOGRAM_OFFSET_ALIGNMENT = 256;

const D3D11_4_VIDEO_DECODER_MAX_HISTOGRAM_COMPONENTS = 4;

const D3D11_8BIT_INDEX_STRIP_CUT_VALUE = 255;

const D3D11_ANISOTROPIC_FILTERING_BIT = 64;

const D3D11_APPEND_ALIGNED_ELEMENT = 4294967295;
...
final D3D11_AUTHENTICATED_CONFIGURE_CRYPTO_SESSION = GUID.fromComponents(
  0x6346cc54,
  0x2cfc,
  0x4ad4,
  Uint8List.fromList(const [0x82, 0x24, 0xd1, 0x58, 0x37, 0xde, 0x77, 0x0]),
);

final D3D11_AUTHENTICATED_CONFIGURE_ENCRYPTION_WHEN_ACCESSIBLE = GUID
    .fromComponents(
      0x41fff286,
      0x6ae0,
      0x4d43,
...
final D3D11_AUTHENTICATED_CONFIGURE_INITIALIZE = GUID.fromComponents(
  0x6114bdb,
  0x3523,
  0x470a,
  Uint8List.fromList(const [0x8d, 0xca, 0xfb, 0xc2, 0x84, 0x51, 0x54, 0xf0]),
);

final D3D11_AUTHENTICATED_CONFIGURE_PROTECTION = GUID.fromComponents(
  0x50455658,
  0x3f47,
  0x4362,
  Uint8List.fromList(const [0xbf, 0x99, 0xbf, 0xdf, 0xcd, 0xe9, 0xed, 0x29]),
);

final D3D11_AUTHENTICATED_CONFIGURE_SHARED_RESOURCE = GUID.fromComponents(
  0x772d047,
  0x1b40,
  0x48e8,
  Uint8List.fromList(const [0x9c, 0xa6, 0xb5, 0xf5, 0x10, 0xde, 0x9f, 0x1]),
);

final D3D11_AUTHENTICATED_QUERY_ACCESSIBILITY_ATTRIBUTES = GUID.fromComponents(
  0x6214d9d2,
  0x432c,
  0x4abb,
  Uint8List.fromList(const [0x9f, 0xce, 0x21, 0x6e, 0xea, 0x26, 0x9e, 0x3b]),
);

final D3D11_AUTHENTICATED_QUERY_CHANNEL_TYPE = GUID.fromComponents(
  0xbc1b18a5,
  0xb1fb,
  0x42ab,
  Uint8List.fromList(const [0xbd, 0x94, 0xb5, 0x82, 0x8b, 0x4b, 0xf7, 0xbe]),
);

final D3D11_AUTHENTICATED_QUERY_CRYPTO_SESSION = GUID.fromComponents(
  0x2634499e,
  0xd018,
  0x4d74,
  Uint8List.fromList(const [0xac, 0x17, 0x7f, 0x72, 0x40, 0x59, 0x52, 0x8d]),
);

final D3D11_AUTHENTICATED_QUERY_CURRENT_ENCRYPTION_WHEN_ACCESSIBLE = GUID
    .fromComponents(
      0xec1791c7,
      0xdad3,
      0x4f15,
...
final D3D11_AUTHENTICATED_QUERY_DEVICE_HANDLE = GUID.fromComponents(
  0xec1c539d,
  0x8cff,
  0x4e2a,
  Uint8List.fromList(const [0xbc, 0xc4, 0xf5, 0x69, 0x2f, 0x99, 0xf4, 0x80]),
);

final D3D11_AUTHENTICATED_QUERY_ENCRYPTION_WHEN_ACCESSIBLE_GUID = GUID
    .fromComponents(
      0xf83a5958,
      0xe986,
      0x4bda,
      Uint8List.fromList(const [0xbe, 0xb0, 0x41, 0x1f, 0x6a, 0x7a, 0x1, 0xb7]),
    );

final D3D11_AUTHENTICATED_QUERY_ENCRYPTION_WHEN_ACCESSIBLE_GUID_COUNT = GUID
    .fromComponents(
      0xb30f7066,
      0x203c,
      0x4b07,
...
final D3D11_AUTHENTICATED_QUERY_OUTPUT_ID = GUID.fromComponents(
  0x839ddca3,
  0x9b4e,
  0x41e4,
  Uint8List.fromList(const [0xb0, 0x53, 0x89, 0x2b, 0xd2, 0xa1, 0x1e, 0xe7]),
);

final D3D11_AUTHENTICATED_QUERY_OUTPUT_ID_COUNT = GUID.fromComponents(
  0x2c042b5e,
  0x8c07,
  0x46d5,
  Uint8List.fromList(const [0xaa, 0xbe, 0x8f, 0x75, 0xcb, 0xad, 0x4c, 0x31]),
);

final D3D11_AUTHENTICATED_QUERY_PROTECTION = GUID.fromComponents(
  0xa84eb584,
  0xc495,
  0x48aa,
  Uint8List.fromList(const [0xb9, 0x4d, 0x8b, 0xd2, 0xd6, 0xfb, 0xce, 0x5]),
);

final D3D11_AUTHENTICATED_QUERY_RESTRICTED_SHARED_RESOURCE_PROCESS = GUID
    .fromComponents(
      0x649bbadb,
      0xf0f4,
      0x4639,
...
final D3D11_AUTHENTICATED_QUERY_RESTRICTED_SHARED_RESOURCE_PROCESS_COUNT = GUID
    .fromComponents(
      0xdb207b3,
      0x9450,
      0x46a6,
...
final D3D11_AUTHENTICATED_QUERY_UNRESTRICTED_PROTECTED_SHARED_RESOURCE_COUNT =
    GUID.fromComponents(
      0x12f0bd6,
      0xe662,
      0x4474,
...
const D3D11_COMPARISON_FILTERING_BIT = 128;

final D3D11_CRYPTO_TYPE_AES128_CTR = GUID.fromComponents(
  0x9b6bd711,
  0x4f74,
  0x41c9,
  Uint8List.fromList(const [0x9e, 0x7b, 0xb, 0xe2, 0xd7, 0xd9, 0x3b, 0x4f]),
);
...
const D3D11_DEBUG_FEATURE_ALWAYS_DISCARD_OFFERED_RESOURCE = 8;

const D3D11_DEBUG_FEATURE_AVOID_BEHAVIOR_CHANGING_DEBUG_AIDS = 64;

const D3D11_DEBUG_FEATURE_DISABLE_TILED_RESOURCE_MAPPING_TRACKING_AND_VALIDATION =
    128;

const D3D11_DEBUG_FEATURE_FINISH_PER_RENDER_OP = 2;

const D3D11_DEBUG_FEATURE_FLUSH_PER_RENDER_OP = 1;

const D3D11_DEBUG_FEATURE_NEVER_DISCARD_OFFERED_RESOURCE = 16;

const D3D11_DEBUG_FEATURE_PRESENT_PER_RENDER_OP = 4;

final D3D11_DECODER_BITSTREAM_ENCRYPTION_TYPE_CBCS = GUID.fromComponents(
  0x422d9319,
  0x9d21,
  0x4bb7,
  Uint8List.fromList(const [0x93, 0x71, 0xfa, 0xf5, 0xa8, 0x2c, 0x3e, 0x4]),
);

final D3D11_DECODER_BITSTREAM_ENCRYPTION_TYPE_CENC = GUID.fromComponents(
  0xb0405235,
  0xc13d,
  0x44f2,
  Uint8List.fromList(const [0x9a, 0xe5, 0xdd, 0x48, 0xe0, 0x8e, 0x5b, 0x67]),
);

final D3D11_DECODER_ENCRYPTION_HW_CENC = GUID.fromComponents(
  0x89d6ac4f,
  0x9f2,
  0x4229,
  Uint8List.fromList(const [0xb2, 0xcd, 0x37, 0x74, 0xa, 0x6d, 0xfd, 0x81]),
);

final D3D11_DECODER_PROFILE_AV1_VLD_12BIT_PROFILE2 = GUID.fromComponents(
  0x17127009,
  0xa00f,
  0x4ce1,
  Uint8List.fromList(const [0x99, 0x4e, 0xbf, 0x40, 0x81, 0xf6, 0xf3, 0xf0]),
);

final D3D11_DECODER_PROFILE_AV1_VLD_12BIT_PROFILE2_420 = GUID.fromComponents(
  0x2d80bed6,
  0x9cac,
  0x4835,
  Uint8List.fromList(const [0x9e, 0x91, 0x32, 0x7b, 0xbc, 0x4f, 0x9e, 0xe8]),
);

final D3D11_DECODER_PROFILE_AV1_VLD_PROFILE0 = GUID.fromComponents(
  0xb8be4ccb,
  0xcf53,
  0x46ba,
  Uint8List.fromList(const [0x8d, 0x59, 0xd6, 0xb8, 0xa6, 0xda, 0x5d, 0x2a]),
);

final D3D11_DECODER_PROFILE_AV1_VLD_PROFILE1 = GUID.fromComponents(
  0x6936ff0f,
  0x45b1,
  0x4163,
  Uint8List.fromList(const [0x9c, 0xc1, 0x64, 0x6e, 0xf6, 0x94, 0x61, 0x8]),
);

final D3D11_DECODER_PROFILE_AV1_VLD_PROFILE2 = GUID.fromComponents(
  0xc5f2aa1,
  0xe541,
  0x4089,
  Uint8List.fromList(const [0xbb, 0x7b, 0x98, 0x11, 0xa, 0x19, 0xd7, 0xc8]),
);

final D3D11_DECODER_PROFILE_H264_IDCT_FGT = GUID.fromComponents(
  0x1b81be67,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
);

final D3D11_DECODER_PROFILE_H264_IDCT_NOFGT = GUID.fromComponents(
  0x1b81be66,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_H264_MOCOMP_FGT = GUID.fromComponents(
  0x1b81be65,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_H264_MOCOMP_NOFGT = GUID.fromComponents(
  0x1b81be64,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_H264_VLD_FGT = GUID.fromComponents(
  0x1b81be69,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_H264_VLD_MULTIVIEW_NOFGT = GUID.fromComponents(
  0x705b9d82,
  0x76cf,
  0x49d6,
  Uint8List.fromList(const [0xb7, 0xe6, 0xac, 0x88, 0x72, 0xdb, 0x1, 0x3c]),
);

final D3D11_DECODER_PROFILE_H264_VLD_NOFGT = GUID.fromComponents(
  0x1b81be68,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_H264_VLD_STEREO_NOFGT = GUID.fromComponents(
  0xf9aaccbb,
  0xc2b6,
  0x4cfc,
  Uint8List.fromList(const [0x87, 0x79, 0x57, 0x7, 0xb1, 0x76, 0x5, 0x52]),
);

final D3D11_DECODER_PROFILE_H264_VLD_STEREO_PROGRESSIVE_NOFGT = GUID
    .fromComponents(
      0xd79be8da,
      0xcf1,
      0x4c81,
...
final D3D11_DECODER_PROFILE_H264_VLD_WITHFMOASO_NOFGT = GUID.fromComponents(
  0xd5f04ff9,
  0x3418,
  0x45d8,
  Uint8List.fromList(const [0x95, 0x61, 0x32, 0xa7, 0x6a, 0xae, 0x2d, 0xdd]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN = GUID.fromComponents(
  0x5b11d51b,
  0x2f4c,
  0x4452,
  Uint8List.fromList(const [0xbc, 0xc3, 0x9, 0xf2, 0xa1, 0x16, 0xc, 0xc0]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN10 = GUID.fromComponents(
  0x107af0e0,
  0xef1a,
  0x4d19,
  Uint8List.fromList(const [0xab, 0xa8, 0x67, 0xa1, 0x63, 0x7, 0x3d, 0x13]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN10_422 = GUID.fromComponents(
  0xbac4fe5,
  0x1532,
  0x4429,
  Uint8List.fromList(const [0xa8, 0x54, 0xf8, 0x4d, 0xe0, 0x49, 0x53, 0xdb]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN10_444 = GUID.fromComponents(
  0xdabeffa,
  0x4458,
  0x4602,
  Uint8List.fromList(const [0xbc, 0x3, 0x7, 0x95, 0x65, 0x9d, 0x61, 0x7c]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN10_EXT = GUID.fromComponents(
  0x9cc55490,
  0xe37c,
  0x4932,
  Uint8List.fromList(const [0x86, 0x84, 0x49, 0x20, 0xf9, 0xf6, 0x40, 0x9c]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN12 = GUID.fromComponents(
  0x1a72925f,
  0xc2c,
  0x4f15,
  Uint8List.fromList(const [0x96, 0xfb, 0xb1, 0x7d, 0x14, 0x73, 0x60, 0x3f]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN12_422 = GUID.fromComponents(
  0x55bcac81,
  0xf311,
  0x4093,
  Uint8List.fromList(const [0xa7, 0xd0, 0x1c, 0xbc, 0xb, 0x84, 0x9b, 0xee]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN12_444 = GUID.fromComponents(
  0x9798634d,
  0xfe9d,
  0x48e5,
  Uint8List.fromList(const [0xb4, 0xda, 0xdb, 0xec, 0x45, 0xb3, 0xdf, 0x1]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN16 = GUID.fromComponents(
  0xa4fbdbb0,
  0xa113,
  0x482b,
  Uint8List.fromList(const [0xa2, 0x32, 0x63, 0x5c, 0xc0, 0x69, 0x7f, 0x6d]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MAIN_444 = GUID.fromComponents(
  0x4008018f,
  0xf537,
  0x4b36,
  Uint8List.fromList(const [0x98, 0xcf, 0x61, 0xaf, 0x8a, 0x2c, 0x1a, 0x33]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MONOCHROME = GUID.fromComponents(
  0x685b993,
  0x3d8c,
  0x43a0,
  Uint8List.fromList(const [0x8b, 0x28, 0xd7, 0x4c, 0x2d, 0x68, 0x99, 0xa4]),
);

final D3D11_DECODER_PROFILE_HEVC_VLD_MONOCHROME10 = GUID.fromComponents(
  0x142a1d0f,
  0x69dd,
  0x4ec9,
  Uint8List.fromList(const [0x85, 0x91, 0xb1, 0x2f, 0xfc, 0xb9, 0x1a, 0x29]),
);

final D3D11_DECODER_PROFILE_JPEG_VLD_420 = GUID.fromComponents(
  0xcf782c83,
  0xbef5,
  0x4a2c,
  Uint8List.fromList(const [0x87, 0xcb, 0x60, 0x19, 0xe7, 0xb1, 0x75, 0xac]),
);

final D3D11_DECODER_PROFILE_JPEG_VLD_422 = GUID.fromComponents(
  0xf04df417,
  0xeee2,
  0x4067,
  Uint8List.fromList(const [0xa7, 0x78, 0xf3, 0x5c, 0x15, 0xab, 0x97, 0x21]),
);

final D3D11_DECODER_PROFILE_JPEG_VLD_444 = GUID.fromComponents(
  0x4cd00e17,
  0x89ba,
  0x48ef,
  Uint8List.fromList(const [0xb9, 0xf9, 0xed, 0xcb, 0x82, 0x71, 0x3f, 0x65]),
);

final D3D11_DECODER_PROFILE_MJPEG_VLD_420 = GUID.fromComponents(
  0x725cb506,
  0xc29,
  0x43c4,
  Uint8List.fromList(const [0x94, 0x40, 0x8e, 0x93, 0x97, 0x90, 0x3a, 0x4]),
);

final D3D11_DECODER_PROFILE_MJPEG_VLD_422 = GUID.fromComponents(
  0x5b77b9cd,
  0x1a35,
  0x4c30,
  Uint8List.fromList(const [0x9f, 0xd8, 0xef, 0x4b, 0x60, 0xc0, 0x35, 0xdd]),
);

final D3D11_DECODER_PROFILE_MJPEG_VLD_444 = GUID.fromComponents(
  0xd95161f9,
  0xd44,
  0x47e6,
  Uint8List.fromList(const [0xbc, 0xf5, 0x1b, 0xfb, 0xfb, 0x26, 0x8f, 0x97]),
);

final D3D11_DECODER_PROFILE_MJPEG_VLD_4444 = GUID.fromComponents(
  0xc91748d5,
  0xfd18,
  0x4aca,
  Uint8List.fromList(const [0x9d, 0xb3, 0x3a, 0x66, 0x34, 0xab, 0x54, 0x7d]),
);

final D3D11_DECODER_PROFILE_MPEG1_VLD = GUID.fromComponents(
  0x6f3ec719,
  0x3735,
  0x42cc,
  Uint8List.fromList(const [0x80, 0x63, 0x65, 0xcc, 0x3c, 0xb3, 0x66, 0x16]),
);

final D3D11_DECODER_PROFILE_MPEG2_IDCT = GUID.fromComponents(
  0xbf22ad00,
  0x3ea,
  0x4690,
  Uint8List.fromList(const [0x80, 0x77, 0x47, 0x33, 0x46, 0x20, 0x9b, 0x7e]),
);

final D3D11_DECODER_PROFILE_MPEG2_MOCOMP = GUID.fromComponents(
  0xe6a9f44b,
  0x61b0,
  0x4563,
  Uint8List.fromList(const [0x9e, 0xa4, 0x63, 0xd2, 0xa3, 0xc6, 0xfe, 0x66]),
);

final D3D11_DECODER_PROFILE_MPEG2_VLD = GUID.fromComponents(
  0xee27417f,
  0x5e28,
  0x4e65,
  Uint8List.fromList(const [0xbe, 0xea, 0x1d, 0x26, 0xb5, 0x8, 0xad, 0xc9]),
);

final D3D11_DECODER_PROFILE_MPEG2and1_VLD = GUID.fromComponents(
  0x86695f12,
  0x340e,
  0x4f04,
  Uint8List.fromList(const [0x9f, 0xd3, 0x92, 0x53, 0xdd, 0x32, 0x74, 0x60]),
);

final D3D11_DECODER_PROFILE_MPEG4PT2_VLD_ADVSIMPLE_GMC = GUID.fromComponents(
  0xab998b5b,
  0x4258,
  0x44a9,
  Uint8List.fromList(const [0x9f, 0xeb, 0x94, 0xe5, 0x97, 0xa6, 0xba, 0xae]),
);

final D3D11_DECODER_PROFILE_MPEG4PT2_VLD_ADVSIMPLE_NOGMC = GUID.fromComponents(
  0xed418a9f,
  0x10d,
  0x4eda,
  Uint8List.fromList(const [0x9a, 0xe3, 0x9a, 0x65, 0x35, 0x8d, 0x8d, 0x2e]),
);

final D3D11_DECODER_PROFILE_MPEG4PT2_VLD_SIMPLE = GUID.fromComponents(
  0xefd64d74,
  0xc9e8,
  0x41d7,
  Uint8List.fromList(const [0xa5, 0xe9, 0xe9, 0xb0, 0xe3, 0x9f, 0xa3, 0x19]),
);

final D3D11_DECODER_PROFILE_VC1_D2010 = GUID.fromComponents(
  0x1b81bea4,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_VC1_IDCT = GUID.fromComponents(
  0x1b81bea2,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_VC1_MOCOMP = GUID.fromComponents(
  0x1b81bea1,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_VC1_POSTPROC = GUID.fromComponents(
  0x1b81bea0,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_VC1_VLD = GUID.fromComponents(
  0x1b81bea3,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_VP8_VLD = GUID.fromComponents(
  0x90b899ea,
  0x3a62,
  0x4705,
  Uint8List.fromList(const [0x88, 0xb3, 0x8d, 0xf0, 0x4b, 0x27, 0x44, 0xe7]),
);

final D3D11_DECODER_PROFILE_VP9_VLD_10BIT_PROFILE2 = GUID.fromComponents(
  0xa4c749ef,
  0x6ecf,
  0x48aa,
  Uint8List.fromList(const [0x84, 0x48, 0x50, 0xa7, 0xa1, 0x16, 0x5f, 0xf7]),
);

final D3D11_DECODER_PROFILE_VP9_VLD_PROFILE0 = GUID.fromComponents(
  0x463707f8,
  0xa1d0,
  0x4585,
  Uint8List.fromList(const [0x87, 0x6d, 0x83, 0xaa, 0x6d, 0x60, 0xb8, 0x9e]),
);

final D3D11_DECODER_PROFILE_WMV8_MOCOMP = GUID.fromComponents(
  0x1b81be81,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_WMV8_POSTPROC = GUID.fromComponents(
  0x1b81be80,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_WMV9_IDCT = GUID.fromComponents(
  0x1b81be94,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_WMV9_MOCOMP = GUID.fromComponents(
  0x1b81be91,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
final D3D11_DECODER_PROFILE_WMV9_POSTPROC = GUID.fromComponents(
  0x1b81be90,
  0xa0c7,
  0x11d3,
  Uint8List.fromList(const [0xb9, 0x84, 0x0, 0xc0, 0x4f, 0x2e, 0x73, 0xc5]),
...
const D3D11_DEFAULT_SAMPLE_MASK = 4294967295;
...
const D3D11_DEFAULT_STENCIL_READ_MASK = 255;

const D3D11_DEFAULT_STENCIL_REFERENCE = 0;

const D3D11_DEFAULT_STENCIL_WRITE_MASK = 255;
...
const D3D11_FILTER_REDUCTION_TYPE_MASK = 3;

const D3D11_FILTER_REDUCTION_TYPE_SHIFT = 7;

const D3D11_FILTER_TYPE_MASK = 3;

const D3D11_FLOAT16_FUSED_TOLERANCE_IN_ULP = 0.6;

const D3D11_FLOAT32_MAX = 3.4028234663852886e+38;

const D3D11_FLOAT32_TO_INTEGER_TOLERANCE_IN_ULP = 0.6000000238418579;

const D3D11_FLOAT_TO_SRGB_EXPONENT_DENOMINATOR = 2.4000000953674316;

const D3D11_FLOAT_TO_SRGB_EXPONENT_NUMERATOR = 1.0;

const D3D11_FLOAT_TO_SRGB_OFFSET = 0.054999999701976776;

const D3D11_FLOAT_TO_SRGB_SCALE_1 = 12.920000076293945;

const D3D11_FLOAT_TO_SRGB_SCALE_2 = 1.0549999475479126;

const D3D11_FLOAT_TO_SRGB_THRESHOLD = 0.0031308000907301903;

const D3D11_FORCE_DEBUGGABLE = 'ForceDebuggable';

const D3D11_FORCE_SHADER_SKIP_OPTIMIZATION = 'ForceShaderSkipOptimization';

const D3D11_FTOI_INSTRUCTION_MAX_INPUT = 2147483648.0;

const D3D11_FTOI_INSTRUCTION_MIN_INPUT = -2147483648.0;

const D3D11_FTOU_INSTRUCTION_MAX_INPUT = 4294967296.0;
...
const D3D11_HS_FORK_PHASE_INSTANCE_COUNT_UPPER_BOUND = 4294967295;
...
const D3D11_HS_JOIN_PHASE_INSTANCE_COUNT_UPPER_BOUND = 4294967295;
...
const D3D11_INFOQUEUE_STORAGE_FILTER_OVERRIDE =
    'InfoQueueStorageFilterOverride';

const D3D11_INFO_QUEUE_DEFAULT_MESSAGE_COUNT_LIMIT = 1024;

const D3D11_INTEGER_DIVIDE_BY_ZERO_QUOTIENT = 4294967295;

const D3D11_INTEGER_DIVIDE_BY_ZERO_REMAINDER = 4294967295;

const D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL = 4294967295;

const D3D11_KEEP_UNORDERED_ACCESS_VIEWS = 4294967295;

final D3D11_KEY_EXCHANGE_HW_PROTECTION = GUID.fromComponents(
  0xb1170d8a,
  0x628d,
  0x4da3,
  Uint8List.fromList(const [0xad, 0x3b, 0x82, 0xdd, 0xb0, 0x8b, 0x49, 0x70]),
);

final D3D11_KEY_EXCHANGE_RSAES_OAEP = GUID.fromComponents(
  0xc1949895,
  0xd72a,
  0x4a1d,
  Uint8List.fromList(const [0x8e, 0x5d, 0xed, 0x85, 0x7d, 0x17, 0x15, 0x20]),
);

const D3D11_LINEAR_GAMMA = 1.0;

const D3D11_MAG_FILTER_SHIFT = 2;
...
const D3D11_MAX_POSITION_VALUE = 3.4028235832468283e+34;
...
const D3D11_MIN_FILTER_SHIFT = 4;

const D3D11_MIN_MAXANISOTROPY = 0;

const D3D11_MIP_FILTER_SHIFT = 0;

const D3D11_MIP_LOD_BIAS_MAX = 15.989999771118164;
...
const D3D11_MULTISAMPLE_ANTIALIAS_LINE_WIDTH = 1.399999976158142;
...
const D3D11_PACKED_TILE = 4294967295;
...
const D3D11_PS_FRONTFACING_DEFAULT_VALUE = 4294967295;

const D3D11_PS_FRONTFACING_FALSE_VALUE = 0;

const D3D11_PS_FRONTFACING_TRUE_VALUE = 4294967295;
...
const D3D11_SDK_LAYERS_VERSION = 1;
...
const D3D11_SHADER_TRACE_FLAG_RECORD_REGISTER_READS = 2;

const D3D11_SHADER_TRACE_FLAG_RECORD_REGISTER_WRITES = 1;
...
const D3D11_SO_DDI_REGISTER_INDEX_DENOTING_GAP = 4294967295;

const D3D11_SO_NO_RASTERIZED_STREAM = 4294967295;
...
const D3D11_SRGB_GAMMA = 2.200000047683716;

const D3D11_SRGB_TO_FLOAT_DENOMINATOR_1 = 12.920000076293945;

const D3D11_SRGB_TO_FLOAT_DENOMINATOR_2 = 1.0549999475479126;

const D3D11_SRGB_TO_FLOAT_EXPONENT = 2.4000000953674316;

const D3D11_SRGB_TO_FLOAT_OFFSET = 0.054999999701976776;

const D3D11_SRGB_TO_FLOAT_THRESHOLD = 0.040449999272823334;
...
const D3D11_TRACE_COMPONENT_W = 8;

const D3D11_TRACE_COMPONENT_X = 1;

const D3D11_TRACE_COMPONENT_Y = 2;

const D3D11_TRACE_COMPONENT_Z = 4;

const D3D11_TRACE_MISC_GS_CUT = 2;

const D3D11_TRACE_MISC_GS_CUT_STREAM = 16;

const D3D11_TRACE_MISC_GS_EMIT = 1;

const D3D11_TRACE_MISC_GS_EMIT_STREAM = 8;

const D3D11_TRACE_MISC_HALT = 32;

const D3D11_TRACE_MISC_MESSAGE = 64;

const D3D11_TRACE_MISC_PS_DISCARD = 4;

const D3D11_TRACE_REGISTER_FLAGS_RELATIVE_INDEXING = 1;
...
const D3DX11_FFT_MAX_DIMENSIONS = 32;

const D3DX11_FFT_MAX_PRECOMPUTE_BUFFERS = 4;

const D3DX11_FFT_MAX_TEMP_BUFFERS = 4;

const D3D_RETURN_PARAMETER_INDEX = -1;

const D3D_SHADER_REQUIRES_11_1_DOUBLE_EXTENSIONS = 32;

const D3D_SHADER_REQUIRES_11_1_SHADER_EXTENSIONS = 64;

const D3D_SHADER_REQUIRES_64_UAVS = 8;

const D3D_SHADER_REQUIRES_DOUBLES = 1;

const D3D_SHADER_REQUIRES_EARLY_DEPTH_STENCIL = 2;

const D3D_SHADER_REQUIRES_LEVEL_9_COMPARISON_FILTERING = 128;

const D3D_SHADER_REQUIRES_MINIMUM_PRECISION = 16;

const D3D_SHADER_REQUIRES_TILED_RESOURCES = 256;

const D3D_SHADER_REQUIRES_UAVS_AT_EVERY_STAGE = 4;
...
const DCOMPOSITION_MAX_WAITFORCOMPOSITORCLOCK_OBJECTS = 32;
...
final DXGI_DEBUG_D3D11 = GUID.fromComponents(
  0x4b99317b,
  0xac39,
  0x4aa6,
  Uint8List.fromList(const [0xbb, 0xb, 0xba, 0xa0, 0x47, 0x84, 0x79, 0x8f]),
);
