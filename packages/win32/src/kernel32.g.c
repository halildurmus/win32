...
BOOL ProcessIdToSessionId_Wrapper(unsigned long dwProcessId, unsigned long* pSessionId) {
  trace_print("Calling ProcessIdToSessionId\n");
  BOOL result_ = ProcessIdToSessionId(dwProcessId, pSessionId);
  trace_print("ProcessIdToSessionId returned %d\n", result_);
  DWORD error = GetLastError();
  setLastError(error);
  trace_print("Set lastError to %d\n", error);
  return result_;
