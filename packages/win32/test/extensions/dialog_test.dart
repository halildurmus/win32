...
const dialogGolden = [
  // DLGTEMPLATE (Expected byte layout)
...
const dialogItemGolden = [
  ...
  // DLGITEMTEMPLATE for [Static Text]
  ...
  // DLGITEMTEMPLATE for [Progress Bar]
...
void main() {
  ...
  group('DLGTEMPLATEPointer extension', () {
    test('set dialog template with custom title and font', () {
      final ptr = adaptiveCalloc<WORD>(40);
      final bytesWritten = ptr.cast<DLGTEMPLATE>().setDialog(
        style:
            WS_POPUP |
            WS_BORDER |
            WS_SYSMENU |
      ...
      check(bytesWritten).equals(dialogGolden.length);
      check(ptr.asTypedList(bytesWritten)).deepEquals(dialogGolden);
      free(ptr);
    });
  });

  group('DLGITEMTEMPLATEPointer extension', () {
    test('set dialog item template with text and progress bar', () {
      final ptr = adaptiveCalloc<WORD>(124);
      var idx = 0;

      // First dialog item: OK Button
      idx += ptr.cast<DLGITEMTEMPLATE>().setDialogItem(
        ...
        cy: 14,
        id: IDOK,
        windowSystemClass: 0x0080, // Button class
        text: 'OK',
      );

      // Second dialog item: Cancel Button
      ...
      check(idx).equals(dialogItemGolden.length);
      check(ptr.asTypedList(idx)).deepEquals(dialogItemGolden);
      free(ptr);
    });

    test('test dialog item template with empty window class', () {
      final ptr = adaptiveCalloc<WORD>(124);
      var idx = 0;
      ...
      // Ensure that the function writes some bytes.
      check(idx).isGreaterThan(0);
