  ...
  group('Win32 API tests', () {
    ...
    group('FindFirstVolume', () {
      test('returns the first volume name', () {
        ...
        final hFindVolume = FindFirstVolume(lpszVolumeName.ptr, MAX_PATH).value;
    ...
    group('GetModuleHandle', () {
      test('returns valid handle for a known module', () {
        ...
        try {
          final hModule = GetModuleHandle(moduleName.ptr).value;
    ...
    group('GetProcAddress', () {
      test('retrieves valid address for a known function', () {
        ...
        try {
          ...
          final pGetProc = GetProcAddress(hModule, procName.ptr).value;
    ...
    group('RegisterClass', () {
      test('registers and unregisters a custom window class successfully', () {
        ...
        try {
          ...
          check(GetClassInfo(hInstance, className.ptr, wc).value).isTrue();
        } finally {
          lpfnWndProc.close();
          free(wc);
          check(UnregisterClass(className.ptr, hInstance).value).isTrue();
          className.free();
        }
      });
    });
