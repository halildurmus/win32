...
void main() {
  ...
  group('utils tests', () {
    ...
    test('free successfully frees calloc-allocated pointer', () {
      ...
      check(refCount(imalloc)).equals(1);
      imalloc.release();
    });

    test('free successfully frees malloc-allocated pointer', () {
      final ptr = adaptiveMalloc<Int32>();
      final imalloc = CoGetMalloc(1);
      check(imalloc).isNotNull();
      check(imalloc!.didAlloc(ptr)).equals(TRUE);
      check(imalloc.getSize(ptr)).equals(sizeOf<Int32>());
    ...
    test('printStruct prints correct memory layout', () {
      final struct = adaptiveCalloc<GUID>()
    ...
    test('wsalloc succeeds', () {
      final ptr = wsalloc(10);
      check(ptr).isNotNull();
      check(ptr.length).isZero();
      check(ptr.toDartString()).isEmpty();
      free(ptr);
    });
  });
}
