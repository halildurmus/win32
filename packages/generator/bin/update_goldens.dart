...
void updateGolden<T extends Projection>(
  ...
  final typeDef = WindowsMetadata.findTypeDef(identifier);
  final projection = projectionFactory(typeDef);
  updateGoldenFile(file, projection);
}

void updateConstantGolden(File file) {
  final constant = WindowsMetadata.findConstant(file.identifier);
...
void updateFunctionGolden(File file) {
  final function = WindowsMetadata.findFunction(file.identifier);
...
''');
  ...
  final goldenFiles =
      Directory(Platform.script.resolve('../test/golden').toFilePath())
