...
void generateCallbacks() {
  final win32CallbacksPath = Platform.script
...
void generateConstants() {
  final win32ConstantsPath = Platform.script
  ...
  final filePath = Platform.script
...
void generateEnums() {
  final win32EnumsPath = Platform.script
...
void generateDynamicLibraryWrappers(ModuleRef library, List<Method> methods) {
  ...
  final hFilePath = Platform.script
      .resolve('../../win32/src/$safeLibraryName.g.h')
      .toFilePath();
  final cFilePath = Platform.script
      .resolve('../../win32/src/$safeLibraryName.g.c')
      .toFilePath();
  final dartFilePath = Platform.script
  ...
  final filteredMethods = functions.values
      .map(
        (f) => methods.singleWhere(
          ...
          orElse: () =>
              throw StateError('Function "${f.originalName}" not found.'),
        ),
      )
...
void generateInterfaces() {
  final comTypesPath = Platform.script
  ...
  for (final interface in interfaces.keys) {
    final typeDef = WindowsMetadata.interfaces
...
void generateStructs() {
  final win32StructsPath = Platform.script
