...
final class EnumProjection extends Projection with ProjectionMixin {
  ...
  EnumProjection(this.typeDef, {super.formatCode})
    : assert(typeDef.kind == TypeKind.enum$, '${typeDef.name} is not an enum.'),
  ...
  List<cb.Field> get fields => typeDef.fields
      ...
      .map(
        (field) => cb.Field(
          (b) => b
            ..docs.addAll(generateApiDocs(apiDetails, tokenObject: field))
  ...
  cb.Library generate() => cb.Library(
    (b) => b.body.addAll([
      cb.ExtensionType(
        (b) => b
          ..docs.addAll(
            generateApiDocs(apiDetails, tokenObject: typeDef, category: 'enum'),
          )
          ..constant = true
          ..name = name
