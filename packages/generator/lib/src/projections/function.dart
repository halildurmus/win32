...
base class FunctionProjection extends Projection with ProjectionMixin {
  ...
  FunctionProjection(this.method)
    : name = method.nameWithoutEncoding.safeTypeName,
      ...
      hint = method.hint,
      originalReturnType = method.returnType.type;
  ...
  String get library {
    try {
      return method.module.safeName;
    } on COMException catch (_) {
      // If this method belongs to a callback, calling `method.module` will
      // throw a COMException. In this case, we return an empty string.
      // It doesn't matter what we return here because when we're generating
      // callbacks, we don't use the `library` getter.
      return '';
    }
  }
  ...
  cb.Method generateMethod() => cb.Method(
    (b) =>
        b
          ...
          ..annotations.addAll([
            if (canBeInlined) generatePreferInlineAnnotation(),
          ])
          ..returns = returnType
          ..name = name
          ..types.addAll([
  ...
  bool get _isAddRefOrRelease =>
      ...
      method.parent.simpleName == 'IUnknown';
