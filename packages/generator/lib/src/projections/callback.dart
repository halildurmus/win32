...
final class CallbackProjection extends Projection with ProjectionMixin {
  ...
  CallbackProjection(this.typeDef)
    : assert(
        typeDef.kind == TypeKind.delegate &&
            typeDef.findMethod('Invoke') != null,
  ...
  FunctionProjection? _cachedFunctionProjection;

  /// The [FunctionProjection] for the callback's `Invoke` method.
  FunctionProjection get functionProjection =>
      _cachedFunctionProjection ??= FunctionProjection(
        typeDef.method('Invoke'),
      );

  /// The return type of the callback.
  cb.Reference get returnType {
    final FunctionProjection(:parameters, originalReturnType: Type(:ffiType)) =
        functionProjection;
  ...
  List<cb.Parameter> get parameters =>
