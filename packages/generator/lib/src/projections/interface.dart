...
base class ComInterfaceProjection extends Projection with ProjectionMixin {
  ...
  late final methodProjections = typeDef.methods
      .map(
        (m) => switch (m) {
  ...
  cb.Reference? get inheritsFrom => typeDef.interfaces.firstOrNull != null
  ...
  List<cb.Directive> get imports => {
    ...coreImports,
    ...methodBasedImports,
    if (inheritedInterfaceImport case final interfaceImport?) interfaceImport,
