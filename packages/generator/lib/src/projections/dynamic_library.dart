...
final class DynamicLibraryProjection extends Projection {
  ...
  /// [methods].
  DynamicLibraryProjection(this.module, this.methods)
  ...
  String? getImportForTypeDef(TypeDef typeDef) => switch (typeDef.kind) {
    ...
    TypeKind.interface => '../com/${typeDef.nameWithoutEncoding.safeFilename}',
    _ => null,
  };
...
final class DynamicLibraryTestProjection extends DynamicLibraryProjection {
  ...
  cb.Block _generateTest(Method method) {
    ...
    // TaskDialog APIs).
    if (function.originalName.startsWith('TaskDialog')) {
      trace('Skipping test for ${method.name}...');
      return cb.Block.of(const []);
    }

    trace('Generating test for ${method.name}...');
