...
/// Represents a Dart projection for a Win32 constant defined by a [Field].
///
...
final class ConstantProjection extends Projection with ProjectionMixin {
  ...
  cb.Spec generate() {
    ...
    return switch (type) {
      ...
      ConstantType.pcwstr => _generateField(
        assignment:
            cb.refer('Pointer<Utf16>').property('fromAddress').call([
              cb.literalNum(field.value! as int),
            ]).code,
        modifier: cb.FieldModifier.final$,
      ),
      ...
      ConstantType.string => _generateField(
        assignment:
            cb
                .literalString((field.value! as String).escapeSpecialChars())
                .code,
      ),
    };
  }
  ...
  cb.Field _generateDevPropKey() {
    final attrValue =
        field.findAttribute(Win32Attribute.constant)!.parameters[0].value
            as String;
    final guidComponents =
        attrValue
    ...
    return _generateField(
      modifier: cb.FieldModifier.final$,
      assignment:
          cb.refer('DevPropKey').property('fromComponents').call([
  ...
  cb.Field _generatePropertyKey() {
    ...
    return _generateField(
      modifier: cb.FieldModifier.final$,
      assignment:
          cb.refer('PropertyKey').property('fromComponents').call([
            ...generateGuidParameters(guidComponents),
            cb.literalNum(pid),
          ]).code,
    );
  ...
  cb.Field _generateSidIdentifierAuthority() {
    ...
    return _generateField(
      modifier: cb.FieldModifier.final$,
      assignment:
          cb.refer('SidIdentifierAuthority').newInstance([
  ...
  cb.Field _generateSignedInt() {
    final type = field.typeIdentifier.name.lastComponent;
...
enum ConstantType {
  ...
  factory ConstantType.fromField(Field field) {
    if (field.existsAttribute(Win32Attribute.guid)) return ConstantType.guid;

    if (field.existsAttribute(Win32Attribute.constant)) {
      final structName = field.typeIdentifier.name.lastComponent;
    ...
    if (field.isLiteral) {
      ...
      switch (field.value) {
        ...
        case int _:
          if (field.typeIdentifier.baseType == BaseType.valueTypeModifier) {
            if (field.type.isVoidPtr) {
              final typeDef = field.typeIdentifier.type!;
              return typeDef.isHandle ? ConstantType.int$ : ConstantType.handle;
            }
          }
