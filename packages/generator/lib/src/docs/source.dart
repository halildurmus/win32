...
/// A [DocsSource] implementation that downloads a NuGet package, extracts a
/// MessagePack file, and decodes its contents.
final class NuGetDocsSource implements DocsSource {
  const NuGetDocsSource({
    required this.package,
    required this.version,
    required this.localStorageManager,
    required this.nugetClient,
  });

  final MetadataPackage package;
  ...
  Future<Map<String, ApiDetails>> loadDocumentation() async {
    ...
    final packagePath = await localStorageManager.getOrDownloadPackage(
      package,
      version,
      () async {
        print('Downloading $package.$version.nupkg...');
    ...
    final msgpackFilePath = p.join(packagePath, 'apidocs.msgpack');
