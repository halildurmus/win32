...
abstract final class DocsSanitizer {
  ...
  /// applies additional pipelines based on the type of [tokenObject].
  static String sanitize(String docs, TokenObject tokenObject) {
    // Apply base transforms.
    var result = _applyTransforms(docs, _baseTransforms);

    // Apply additional transforms based on the type of token.
    if (tokenObject is Field) {
      final parent = tokenObject.parent;
      if (parent.isEnum) {
        ...
        if (tokenObject.isCharArray) {
          result = _applyTransforms(result, _charArrayTransforms);
        }
      }
    } else if (tokenObject is Method) {
