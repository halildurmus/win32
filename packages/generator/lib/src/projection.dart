...
abstract class Projection {
  ...
  @override
  String toString() {
    logger.finest('Calling generate() for $debugName...');
    final sink = generate().accept(_emitter);
    if (trailingNewline) sink.writeln();
    final rawCode = sink.toString();

    if (formatCode) {
      logger.finest('Formatting code for $debugName...');
      try {
        return DartFormatter(
          languageVersion: DartFormatter.latestLanguageVersion,
        ).format(rawCode);
      } catch (e) {
        logger.severe('Failed to format code for $debugName.\n$e');
        return rawCode;
      }
    }
...
mixin ProjectionMixin on Projection {
  ...
  List<String> generateApiDocs(
    ...
    required TokenObject tokenObject,
    List<String> additionalDocs = const [],
    String? category,
  }) {
    logger.finest('Writing documentation for $debugName...');
    if (apiDetails == null) {
      logger.finest('No documentation found for $debugName.');
    }

    final isEnumOrStructField =
        tokenObject is Field &&
        (tokenObject.parent.isEnum || tokenObject.parent.isStruct);
    var description =
        isEnumOrStructField
            ? apiDetails?.fields[tokenObject.name]
    ...
    if (description != null) {
      ...
      description = DocsSanitizer.sanitize(description, tokenObject);
    }
