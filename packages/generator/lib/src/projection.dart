...
abstract class Projection {
  ...
  @override
  String toString() {
    trace('Calling generate() for $debugName...');
    final sink = generate().accept(_emitter);
    if (trailingNewline) sink.writeln();
    final rawCode = sink.toString();

    if (formatCode) {
      trace('Formatting code for $debugName...');
      try {
        return DartFormatter(
          languageVersion: DartFormatter.latestLanguageVersion,
        ).format(rawCode);
      } catch (e) {
        error('Failed to format code for $debugName.\n$e');
        return rawCode;
      }
    }
...
mixin ProjectionMixin on Projection {
  ...
  List<String> generateApiDocs(
    ...
    trace('Writing documentation for $debugName...');
    if (apiDetails == null) {
      trace('No documentation found for $debugName.');
    }
