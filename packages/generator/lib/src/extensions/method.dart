...
extension MethodHelpers on Method {
  ...
  bool get canBeProjectedAsGetter {
    ...
    if (isGetProperty && parameters.length == 1) {
      ...
      if (getterProjection.deref() case Type(
        isStruct: true,
  ...
  /// interpreted.
  ReturnHint get hint {
    if (existsAttribute(Win32Attribute.canReturnMultipleSuccessValues)) {
      return ReturnHint.none;
    }

    final outParameters = parameters.where((p) => p.isOutParam).toFixedList();
    final returnValue =
        outParameters.length == 1 &&
                parameters.last.token == outParameters[0].token
            ? outParameters[0]
            : returnType;
    ...
    return switch (returnType.type) {
      ...
      HRESULTType() when isQuery != null =>
        isQuery!.object.isOptional
            ...
            : ReturnHint.query,
      HRESULTType() =>
        returnValue.isRetVal ? ReturnHint.resultValue : ReturnHint.resultVoid,
      TypeDefType(
        typeDef: TypeDef(kind: TypeKind.struct, isWrapperStruct: false),
      ) =>
  ...
  bool get isObsolete => existsAttribute(SystemAttribute.obsolete);
  ...
  ({Parameter guid, int guidIdx, Parameter object, int objectIdx})?
  get isQuery {
    final parameters = this.parameters;
    if (parameters.length < 2) return null;

    final guidIdx = parameters.lastIndexWhere(
      (p) => p.type == const Type.constPtr(GUIDType(), 1) && !p.isOutParam,
    );
    ...
    final object =
        parameters
            .where(
              (p) =>
                  p.type == const Type.mutPtr(Type.void$, 2) &&
                  p.existsAttribute(Win32Attribute.comOutPtr),
            )
            .singleOrNull;
    if (object == null) return null;
    final objectIdx = parameters.indexWhere((p) => p.token == object.token);
  ...
  bool get isRetVal {
    final lastParam = parameters.lastOrNull;
    // First we check whether there's an explicit retval parameter.
    if (lastParam?.existsAttribute(Win32Attribute.retVal) ?? false) {
      return true;
    }

    // Otherwise we check for heuristically for additional retval candidates.
    final previousParams = parameters.take(parameters.length - 1);
  ...
  /// Finds and returns a parameter by its name.
  ...
  bool get supportsLastError {
    try {
      return pinvokeMap.supportsLastError;
    } catch (_) {
      return false;
    }
  }
  ...
  String get uniqueName {
    ...
    while (interfaceTypeDef.interfaces.isNotEmpty &&
        interfaceTypeDef.interfaces.first.typeName != TypeName.IUnknown) {
