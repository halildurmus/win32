...
extension TypeDefExtension on winmd.TypeDef {
  ...
  winmd.MethodDef? get freeFunction {
    ...
    if (raiiFree.fixedArgs case [
      winmd.FixedArg(value: winmd.Utf8StringValue(:final value)),
    ]) {
  ...
  Int64List get invalidHandleValues {
    ...
    for (var i = 0; i < invalidHandleValueAttributes.length; i++) {
      ...
      if (attr.fixedArgs case [
        winmd.FixedArg(value: winmd.Int64Value(:final value)),
      ]) {
        values[i] = value;
      }
    }
  ...
  bool get isHandle {
    if (fields case [final field] when field.name == 'Value') {
      final type = field.type;
      return type.isVoidPtr || type.isPrimitive;
    }
