...
void printProjection<T>(
  ...
  final typeDef = WindowsMetadata.findTypeDef(type);
...
void printConstant([String name = 'HKEY_LOCAL_MACHINE']) {
  final constant = WindowsMetadata.findConstant(name);
...
void printFunction([String name = 'BroadcastSystemMessageW']) {
  final function = WindowsMetadata.findFunction(name);
...
void printGetProperty([
  ...
  final typeDef = WindowsMetadata.findTypeDef(interface);
...
void main() async {
  ...
  final constants = WindowsMetadata.constants;
  final delegates = WindowsMetadata.delegates;
  final enums = WindowsMetadata.enums;
  final functions = WindowsMetadata.functions;
  final interfaces = WindowsMetadata.interfaces;
  final structs = WindowsMetadata.structs;
  print('Constants: ${constants.length}');
  print('Delegates: ${delegates.length}');
  print('Enums: ${enums.length}');
  print('Functions: ${functions.length}');
  print('Interfaces: ${interfaces.length}');
  print('Structs: ${structs.length}');
