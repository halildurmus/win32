...
void testMethodParameterType(
  ...
  test('$methodName.$parameterName parameter', () {
    final method = WindowsMetadata.findFunction(methodName);
    final parameter =
        method.parameters.where((p) => p.name == parameterName).firstOrNull;
    check(
      parameter,
      because:
          '"$parameterName" parameter is not found in the "$methodName" '
          'method.',
    ).isNotNull();
    type(Type.fromTypeIdentifier(parameter!.typeIdentifier));
  });
}

@isTest
void testMethodReturnType(String methodName, void Function(Type) type) {
  test('$methodName return', () {
    final method = WindowsMetadata.findFunction(methodName);
    type(Type.fromTypeIdentifier(method.returnType.typeIdentifier));
  });
}
...
void testStructFieldType(
  ...
  test('$struct.$fieldName field', () {
    final typeDef = WindowsMetadata.findTypeDef(struct);
    final field = typeDef.field(fieldName);
    type(Type.fromTypeIdentifier(field.typeIdentifier));
  });
}
