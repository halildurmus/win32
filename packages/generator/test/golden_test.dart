...
void testCallbackGolden(String callbackName) {
  test(callbackName, () {
    final typeDef = WindowsMetadata.findTypeDef(callbackName);
...
void testConstantGolden(String constantName) {
  test(constantName, () {
    final constant = WindowsMetadata.findConstant(constantName);
...
void testDynamicLibraryGolden(String libraryName, {bool isWdk = false}) {
  test(libraryName, () {
    ...
    final scope = isWdk ? WindowsMetadata.wdk : WindowsMetadata.win32;
...
void testEnumGolden(String enumName) {
  test(enumName, () {
    final typeDef = WindowsMetadata.findTypeDef(enumName);
    final fileName = typeDef.nameWithoutEncoding.safeFilename;
...
void testFunctionGolden(String functionName) {
  test(functionName, () {
    final function = WindowsMetadata.findFunction(functionName);
...
void testInterfaceGolden(String interfaceName) {
  test(interfaceName, () {
    final typeDef = WindowsMetadata.findTypeDef(interfaceName);
    final fileName = typeDef.nameWithoutEncoding.safeFilename;
...
void testMethodGolden(String parent, String methodName) {
  test('$parent.$methodName', () {
    final typeDef = WindowsMetadata.findTypeDef(parent);
...
void testStructGolden(String structName) {
  test(structName, () {
    final typeDef = WindowsMetadata.findTypeDef(structName);
    final fileName = typeDef.nameWithoutEncoding.safeFilename;
