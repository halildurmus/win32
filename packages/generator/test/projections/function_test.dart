...
void main() {
  ...
  group('FunctionProjection', () {
    testFunction('NtQueryObject', (projection) {
      ...
      ).equals('int Function(Pointer, int, Pointer, int, Pointer<Uint32>)');
      check(
    ...
    testFunction('midiConnect', (projection) {
      ...
      ).equals('Uint32 Function(Pointer, Pointer, Pointer)');
      check(
        projection.dartPrototype.accept(DartEmitter()).toString(),
      ).equals('int Function(Pointer, Pointer, Pointer)');
      check(
        projection.methodParameters.map(
          (p) => '${p.dartType.symbol} ${p.name}',
        ),
      ).deepEquals(['HMIDI hmi', 'HMIDIOUT hmo']);
      check(
        projection.arguments.map((a) => a.accept(DartEmitter()).toString()),
    ...
    testFunction('WindowsConcatString', (projection) {
      ...
      check(
        projection.nativePrototype.accept(DartEmitter()).toString(),
      ).equals('Int32 Function(Pointer, Pointer, Pointer<Pointer>)');
      check(
        projection.dartPrototype.accept(DartEmitter()).toString(),
      ).equals('int Function(Pointer, Pointer, Pointer<Pointer>)');
      check(
        projection.methodParameters.map(
          (p) => '${p.dartType.symbol} ${p.name}',
        ),
      ).deepEquals(['HSTRING? string1', 'HSTRING? string2']);
      check(
        projection.arguments.map((a) => a.accept(DartEmitter()).toString()),
      ).deepEquals(['string1 ?? nullptr', 'string2 ?? nullptr', 'newString']);
    ...
    testFunction('ChooseFontW', (projection) {
      ...
      check(
        projection.nativePrototype.accept(DartEmitter()).toString(),
      ).equals('Int32 Function(Pointer<CHOOSEFONT>)');
      check(
        projection.dartPrototype.accept(DartEmitter()).toString(),
