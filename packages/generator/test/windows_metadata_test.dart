  ...
  group('WindowsMetadata', () {
    ...
    group('findConstant', () {
      test('finds constant by name', () {
        check(
          WindowsMetadata.findConstant('E_NOINTERFACE').name,
        ).equals('E_NOINTERFACE');
      });

      test('throws StateError for non-existent constant', () {
        check(
          () => WindowsMetadata.findConstant('NonExistentConstant'),
        ).throws<StateError>();
      });
    });
    ...
    group('findFunction', () {
      test('finds function by name', () {
        check(
          WindowsMetadata.findFunction('CreateFileW').name,
        ).equals('CreateFileW');
      });

      test('throws StateError for non-existent function', () {
        check(
          () => WindowsMetadata.findFunction('NonExistentFunction'),
        ).throws<StateError>();
      });
    });
    ...
    group('findTypeDef', () {
      test('finds typeDef by name', () {
        check(
          WindowsMetadata.findTypeDef('IUnknown').simpleName,
        ).equals('IUnknown');
      });

      test('throws StateError for non-existent typeDef', () {
        check(
          () => WindowsMetadata.findTypeDef('NonExistentTypeDef'),
        ).throws<StateError>();
      });
    });
  });
