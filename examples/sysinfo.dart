...
int getSystemMemoryInMegabytes() {
  ...
  try {
    ...
    if (value) return memory.value ~/ 1024;
...
void printPowerInfo() {
  ...
  try {
    final Win32Result(:value, :error) = GetSystemPowerStatus(powerStatus);
    if (value) {
...
String getUserName() {
  ...
  try {
    final Win32Result(:value, :error) = GetUserName(lpBuffer.ptr, pcbBuffer);
    if (value) return lpBuffer.toDartString();
    throw WindowsException(error.toHRESULT());
  } finally {
    free(pcbBuffer);
  }
