...
final int hInstance = GetModuleHandle(null).value;
...
int mainWindowProc(int hwnd, int message, int wParam, int lParam) {
  switch (message) {
    case WM_CREATE:
      ...
      messageFindReplace = RegisterWindowMessage(lpString.ptr).value;
      ...
      switch (lParam) {
        case 1: // Edit menu
          ...
          EnableMenuItem(
            hMenu,
            IDM_EDIT_UNDO,
            SendMessage(hwndEdit, EM_CANUNDO, 0, 0).value != FALSE
                ? MF_ENABLED
                : MF_GRAYED,
          );
...
void main() {
  ...
  if (hWnd == NULL) throw WindowsException(error.toHRESULT());
  ...
  while (GetMessage(msg, null, 0, 0).value) {
    ...
    if (hDlgModeless == NULL || !IsDialogMessage(hDlgModeless, msg)) {
      ...
      if (TranslateAccelerator(hWnd, hAccel, msg).value == FALSE) {
