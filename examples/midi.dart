...
void main() {
  ...
  final mciStatusParams =
      loggingCalloc<MCI_STATUS_PARMS>()..ref.dwItem = MCI_SEQ_STATUS_PORT;
  ...
  if (LOWORD(mciStatusParams.ref.dwReturn) != MIDI_MAPPER) {
    if (MessageBox(
          null,
          w('The MIDI mapper is not available. Continue?').ptr,
          null,
          MB_YESNO,
        ) ==
        IDNO) {
  ...
  MessageBox(null, w('Press OK to stop').ptr, w('Midi Sample').ptr, MB_OK);
