...
class DialogBuilder {
  ...
  String? show() {
    final ptr = loggingCalloc<Uint16>(_bufferSize);
    try {
      var idx = 0;
  ...
  int _dialogProc(int hwndDlg, int message, int wParam, int lParam) {
    switch (message) {
      case WM_INITDIALOG:
  ...
  String? _getEditText(int hwndDlg) {
    final textPtr = Pwstr.allocate(_textBufferSize);
