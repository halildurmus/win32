...
class PieceSet {
  PieceSet() {
    ...
    // 0, I piece, red
    tetrimino = <Point>[Point(), Point(0, 1), Point(0, 2), Point(0, 3)];
    pieces[0].add(Piece(0, 0, Colors.red, tetrimino));

    // 1, L piece, orange
    tetrimino = <Point>[Point(), Point(1), Point(0, 1), Point(0, 2)];
    pieces[1].add(Piece(1, 0, Colors.orange, tetrimino));

    // 2, counter-L piece, yellow
    tetrimino = <Point>[Point(), Point(1), Point(1, 1), Point(1, 2)];
    pieces[2].add(Piece(2, 0, Colors.yellow, tetrimino));

    // 3, S piece, green
    tetrimino = <Point>[Point(), Point(1), Point(1, 1), Point(2, 1)];
    pieces[3].add(Piece(3, 0, Colors.green, tetrimino));

    // 4, Z piece, blue
    tetrimino = <Point>[Point(1), Point(2), Point(0, 1), Point(1, 1)];
    pieces[4].add(Piece(4, 0, Colors.blue, tetrimino));

    // 5, Square piece, indigo
    tetrimino = <Point>[Point(), Point(1), Point(0, 1), Point(1, 1)];
    pieces[5].add(Piece(5, 0, Colors.indigo, tetrimino));

    // 6, T piece, violet
    tetrimino = <Point>[Point(), Point(1), Point(2), Point(1, 1)];
