// ID3D12VideoProcessCommandList1.dart

// THIS FILE IS GENERATED AUTOMATICALLY AND SHOULD NOT BE EDITED DIRECTLY.

// ignore_for_file: unused_import, directives_ordering

import 'dart:ffi';

import 'package:ffi/ffi.dart';

import '../../combase.dart';
import '../../constants.dart';
import '../../exceptions.dart';
import '../../guid.dart';
import '../../macros.dart';
import '../../ole32.dart';
import '../../utils.dart';

import '../../media/mediafoundation/ID3D12VideoProcessCommandList.dart';
import '../../media/mediafoundation/ID3D12VideoProcessor.dart';
import '../../media/mediafoundation/structs.g.dart';

/// @nodoc
const IID_ID3D12VideoProcessCommandList1 =
    '{542C5C4D-7596-434F-8C93-4EFA6766F267}';

/// {@category Interface}
/// {@category com}
class ID3D12VideoProcessCommandList1 extends ID3D12VideoProcessCommandList {
  // vtable begins at 23, is 1 entries long.
  ID3D12VideoProcessCommandList1(Pointer<COMObject> ptr) : super(ptr);

  void ProcessFrames1(
    Pointer<COMObject> pVideoProcessor,
    Pointer<D3D12_VIDEO_PROCESS_OUTPUT_STREAM_ARGUMENTS> pOutputArguments,
    int NumInputStreams,
    Pointer<D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1> pInputArguments,
  ) =>
      ptr.ref.lpVtbl.value
          .elementAt(23)
          .cast<
              Pointer<
                  NativeFunction<
                      Void Function(
            Pointer,
            Pointer<COMObject> pVideoProcessor,
            Pointer<D3D12_VIDEO_PROCESS_OUTPUT_STREAM_ARGUMENTS>
                pOutputArguments,
            Uint32 NumInputStreams,
            Pointer<D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1>
                pInputArguments,
          )>>>()
          .value
          .asFunction<
              void Function(
            Pointer,
            Pointer<COMObject> pVideoProcessor,
            Pointer<D3D12_VIDEO_PROCESS_OUTPUT_STREAM_ARGUMENTS>
                pOutputArguments,
            int NumInputStreams,
            Pointer<D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1>
                pInputArguments,
          )>()(
        ptr.ref.lpVtbl,
        pVideoProcessor,
        pOutputArguments,
        NumInputStreams,
        pInputArguments,
      );
}
