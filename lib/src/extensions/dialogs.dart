...
extension DialogTemplateHelper on Pointer<DLGTEMPLATE> {
  ...
  int setDialog(
    ...
    // identifies a menu resource for the dialog box. If the first element of this
    // array is 0x0000, the dialog box has no menu and the array has no other
    // elements. Since Dart does not currently support embedding a resource file
    // in a Windows executable, the other settings are moot.
    ptr[idx++] = 0x0000;

    // Following the menu array is a class array that identifies the window class
    // of the dialog box. If the first element of the array is 0x0000, the system
    // uses the predefined dialog box class for the dialog box and the array has
    // no other elements. If the first element is 0xFFFF, the array has one
    // additional element that specifies the ordinal value of a predefined system
    // window class. If the first element has any other value, the system treats
    // the array as a null-terminated Unicode string that specifies the name of a
...
extension DialogItemTemplateHelper on Pointer<DLGITEMTEMPLATE> {
  ...
  int setDialogItem({
    ...
    /// Immediately following each DLGITEMTEMPLATE structure is a class array that
    /// specifies the window class of the control. If the first element of this
    /// array is any value other than 0xFFFF, the system treats the array as a
    /// null-terminated Unicode string that specifies the name of a registered
    /// window class. If the first element is 0xFFFF, the array has one additional
    /// element that specifies the ordinal value of a predefined system class.
    ///
    ...
    // array. This creation data can be of any size and format. If the first word
    // of the creation data array is nonzero, it indicates the size, in bytes, of
    // the creation data (including the size word).
    if (creationDataBytes.isNotEmpty) {
      ...
      dataBlock.setAll(0, creationDataBytes);
